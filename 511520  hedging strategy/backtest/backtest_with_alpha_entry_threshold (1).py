...
def backtest_spread_strategy(
    ...
    alpha_etf_units = 0
    alpha_fut_units = 0
    alpha_trade_log.append({
                    "date": date,
                    "action": "close",
                    "etf_units": alpha_etf_units,
                    "fut_units": alpha_fut_units,
                    "etf_price": open_etf,
                    "fut_price": open_fut,
                    "equity": total_value_now,
                    "pnl_rate": alpha_pnl_rate
                })
    ...
    for idx in range(len(df)):
        ...
        if signal_today < alpha_entry_threshold and hedge_ratio > 0:
            if alpha_etf_units < 2 * base_etf_units:
                ...
                entry_equity_alpha = cash + (base_etf_units + alpha_etf_units)
                alpha_trade_log.append({
                    "date": date,
                    "action": "open",
                    "etf_units": alpha_etf_units,
                    ...
                    "etf_price": open_etf,
                    "fut_price": open_fut,
                    "equity": entry_equity_alpha
                }) * open_etf - (base_fut_units + alpha_fut_units) * open_fut * 10000
    ...
    return result_df, metrics, realized_pnls, alpha_trade_log